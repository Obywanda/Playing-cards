...
export class MonsterService {
  ...
  private  init(){
    ...
    monster3.hp = 30;
    monster3.figureCaption ="N°004 Bulbi";
    this.monsters.push(monster3);

    const monster4 = new Monster();
    monster4.id = this.currentIndex++;
    monster4.name = "Draco";
    monster4.image = "/img/draco.png";
    monster4.type = MonsterType.FIRE;
    monster4.hp = 160;
    monster4.figureCaption ="N°005 Draco";
  ...
  update(monster: Monster): Monster{
    ...
    const monsterIndex = this.monsters.findIndex(orginalMonster => orginalMonster.id === monster.id);
    if(monsterIndex){
  ...
  delete(id: number) {
    const monsterIndex =  this.monsters.findIndex(orginalMonster => orginalMonster.id === id);
